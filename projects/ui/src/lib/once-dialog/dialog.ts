...
export const OUI_DIALOG_DEFAULT_OPTIONS =
    new InjectionToken<OuiDialogConfig>('oui-dialog-default-options');

/** Injection token that determines the scroll handling while the dialog is open. */
export const OUI_DIALOG_SCROLL_STRATEGY =
    new InjectionToken<() => ScrollStrategy>('oui-dialog-scroll-strategy');

/** @docs-private */
export function OUI_DIALOG_SCROLL_STRATEGY_FACTORY(overlay: Overlay): () => ScrollStrategy {
    return () => overlay.scrollStrategies.reposition();
}

/** @docs-private */
export function OUI_DIALOG_SCROLL_STRATEGY_PROVIDER_FACTORY(overlay: Overlay):
    () => ScrollStrategy {
    return () => overlay.scrollStrategies.reposition();
}
...
export class OuiDialog implements OnDestroy {
    ...
    constructor(
        ...
        @Optional() @Inject(OUI_DIALOG_DEFAULT_OPTIONS) private _defaultOptions: OuiDialogConfig,
        @Inject(OUI_DIALOG_SCROLL_STRATEGY) scrollStrategy: any,
        @Optional() @SkipSelf() private _parentDialog: OuiDialog,
    ...
    open<T, D = any, R = any>(componentOrTemplateRef: ComponentType<T> | TemplateRef<T>,
        config?: OuiDialogConfig<D>): OuiDialogRef<T, R> {

        config = _applyConfigDefaults(config, this._defaultOptions || new OuiDialogConfig());
     ...
     */
    private _attachDialogContainer(overlay: OverlayRef, config: OuiDialogConfig): OuiDialogContainer {

        const userInjector = config && config.viewContainerRef && config.viewContainerRef.injector;
        const injector = new PortalInjector(userInjector || this._injector, new WeakMap([
            [OuiDialogConfig, config]
        ]));
        const containerPortal =
            new ComponentPortal(OuiDialogContainer, config.viewContainerRef, injector);
    ...
    private _attachDialogContent<T, R>(
        ...
        config: OuiDialogConfig): OuiDialogRef<T, R> {
        // Create a reference to the dialog we're creating in order to give the user a handle
        // to modify and close it.
        const dialogRef =
            new OuiDialogRef<T, R>(overlayRef, dialogContainer, config.id);
    ...
    private _createInjector<T>(
        ...
        dialogContainer: OuiDialogContainer): PortalInjector {
...
function _applyConfigDefaults(
    config?: OuiDialogConfig, defaultOptions?: OuiDialogConfig): OuiDialogConfig {
