...
<Canvas>
  ...
  >
    ...
    <div style="width: 213px;">
    <oui-form-field [appearance]="appearance">
        <oui-select ngClass="{{theme}}" (selectionChange)="onChange($event)" [large]="large" [placeholder]="placeholder" [disabled]="disabled" singleActionItem [singleActionLabel]="singleActionLabel">
           <oui-select-search [(ngModel)]="keyword"></oui-select-search>
          <oui-option>-- None --</oui-option>
          <oui-optgroup *ngFor="let group of groups" [label]="group.name">
            <oui-option *ngFor="let option of (group.options | filterOptions: keyword) " [value]="option">
              {{option}}
            </oui-option>
          </oui-optgroup>
        </oui-select>
