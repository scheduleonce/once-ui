...
<Canvas>
  ...
  >
    ...
    <div style="width: 213px;">
    <oui-form-field [appearance]="appearance">
        <oui-select (selectionChange)="onChange($event)" ngClass="{{theme}}" [large]="large" [placeholder]="placeholder" multiple actionItems [disabled]="disabled">
          <oui-option *ngFor="let option of options" [value]="option">
            {{option}}
          </oui-option>
          <!--div class="oui-select-action-wrapper"><div class="oui-select-action-items"><button oui-link-button color="primary">Cancel</button><button oui-button color="primary">Done</button></div></div-->
        </oui-select>
      </oui-form-field>
    </div>
    `,
