...
export function runTabNavBarHarnessTests(
  ...
  it('should throw error when attempting to click invalid link', async () => {
    ...
    await expectAsync(navBar.clickLink({label: 'Fake'})).toBeRejectedWithError(
      /Cannot find oui-tab-link matching filter {"label":"Fake"}/,
    );
  });
...
@Component({
  template: `
    <nav oui-tab-nav-bar [tabPanel]="tabPanel">
      <a href="#" (click)="select(0, $event)" [active]="activeLink === 0" ouiTabLink>First</a>
      <a href="#" (click)="select(1, $event)" [active]="activeLink === 1" ouiTabLink>Second</a>
      <a
        href="#"
    ...
    <oui-tab-nav-panel #tabPanel id="tab-panel">
      Tab content
    </oui-tab-nav-panel>
  `,
})
