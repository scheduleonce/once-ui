...
export function runTabGroupHarnessTests(
  ...
  it('should throw error when attempting to select invalid tab', async () => {
    ...
    await expectAsync(tabGroup.selectTab({label: 'Fake'})).toBeRejectedWithError(
      /Cannot find oui-tab matching filter {"label":"Fake"}/,
    );
  });
...
@Component({
  template: `
    <oui-tab-group>
      <oui-tab label="First" aria-label="First tab">
        <span class="test-tab-content">Content 1</span>
      </oui-tab>
      <oui-tab label="Second" aria-label="Second tab">
        <span class="test-tab-content">Content 2</span>
      </oui-tab>
      <oui-tab label="Third" aria-labelledby="tabLabelId" [disabled]="isDisabled">
        <ng-template ouiTabLabel>Third</ng-template>
