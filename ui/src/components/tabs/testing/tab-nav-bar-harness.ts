...
import {OuiTabLinkHarness} from './tab-link-harness';
import {OuiTabNavPanelHarness} from './tab-nav-panel-harness';

/** Harness for interacting with an MDC-based oui-tab-nav-bar in tests. */
export class OuiTabNavBarHarness extends ComponentHarness {
  /** The selector for the host element of a `OuiTabNavBar` instance. */
  static hostSelector = '.oui-mdc-tab-nav-bar';
   ...
   */
  static with<T extends OuiTabNavBarHarness>(
   ...
   */
  async getLinks(filter: TabLinkHarnessFilters = {}): Promise<OuiTabLinkHarness[]> {
    return this.locatorForAll(OuiTabLinkHarness.with(filter))();
  }

  /** Gets the active link in the nav bar. */
  async getActiveLink(): Promise<OuiTabLinkHarness> {
  ...
  async clickLink(filter: TabLinkHarnessFilters = {}): Promise<void> {
    ...
    if (!tabs.length) {
      throw Error(`Cannot find oui-tab-link matching filter ${JSON.stringify(filter)}`);
    }
    await tabs[0].click();
  }

  /** Gets the panel associated with the nav bar. */
  async getPanel(): Promise<OuiTabNavPanelHarness> {
    ...
    return await this.documentRootLocatorFactory().locatorFor(OuiTabNavPanelHarness.with(filter))();
  }
}
