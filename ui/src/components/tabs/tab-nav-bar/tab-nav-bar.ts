...
/**
 * Base class with all of the `OuiTabNav` functionality.
 * @docs-private
 */
@Directive()
export abstract class _OuiTabNavBase
  ...
  implements AfterContentChecked, AfterContentInit, OnDestroy
{
  ...
  set backgroundColor(value: ThemePalette) {
    ...
    classList.remove(`oui-background-${this.backgroundColor}`);

    if (value) {
      classList.add(`oui-background-${value}`);
    }
...
const _OuiTabLinkBaseWithInkBarItem = mixinInkBarItem(_OuiTabLinkBase);
