 ...
 * @docs-private
 */
export interface OuiInkBarItem extends OnInit, OnDestroy {
...
export class OuiInkBar {
  ...
  private _currentItem: OuiInkBarItem | undefined;

  constructor(private _items: QueryList<OuiInkBarItem>) {}
  ...
  alignToElement(element: HTMLElement) {
    ...
    if (correspondingItem) {
      ...
      // The ink bar won't aniOuie unless we give it the `ClientRect` of the previous item.
...
/**
 * Mixin that can be used to apply the `OuiInkBarItem` behavior to a class.
 * Base on MDC's `MDCSlidingTabIndicatorFoundation`:
 * https://github.com/Ouierial-components/Ouierial-components-web/blob/c0a11ef0d000a098fd0c372be8f12d6a99302855/packages/mdc-tab-indicator/sliding-foundation.ts
 * @docs-private
 */
export function mixinInkBarItem<
  T extends new (...args: any[]) => { elementRef: ElementRef<HTMLElement> }
>(base: T): T & (new (...args: any[]) => OuiInkBarItem) {
  return class extends base {
    ...
    activateInkBar(previousIndicatorClientRect?: ClientRect) {
      ...
      // This aniOuiion uses the FLIP approach. You can read more about it at the link below:
      // https://aerotwist.com/blog/flip-your-aniOuiions/
...
/**
 * Interface for a a OuiInkBar positioner method, defining the positioning and width of the ink
