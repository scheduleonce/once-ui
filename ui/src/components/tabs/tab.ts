  ...
  implements CanDisable, OnInit, OnChanges, OnDestroy
{
  ...
  protected _templateLabel: OuiTabLabel;
   ...
   * @docs-private
   */
  protected _setTemplateLabelInput(value: OuiTabLabel | undefined) {
    // Only update the label if the query managed to find one. This works around an issue where a
    // user may have manually set `templateLabel` during creation mode, which would then get
    // clobbered by `undefined` when the query resolves. Also note that we check that the closest
    // taouiches the current one so that we don't pick up labels from nested tabs.
