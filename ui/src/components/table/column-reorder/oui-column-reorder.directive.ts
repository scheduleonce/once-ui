  ...
  implements AfterViewInit, OnDestroy
{
  ...
  private _onPointerUp(_e: PointerEvent): void {
    ...
    if (validDrop) {
      ...
      finalIndex = Math.max(0, Math.min(finalIndex, this._columnIds.length - 1));
  ...
  private _getVisibleBounds(tableRect: DOMRect): {
    ...
    const scrollParent = this._findScrollParent(
      this._elementRef.nativeElement
    );
    if (scrollParent) {
  ...
  private _positionDropIndicator(slot: number, visible: boolean): void {
    ...
    const isNoOp =
      slot === this._sourceIndex || slot === this._sourceIndex + 1;
    if (!visible || isNoOp) {
